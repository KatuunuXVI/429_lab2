...
int main(int argc, char*argv[]) {
    ...
    /*struct file_info sample_text;
    printf("Create FIle: %d\n",create_file(&sample_text,"sample.txt"));
    int i = 0;
    char* text = malloc(15);
    text = "This is a line\n";
    while(sample_text.file_len < 24000000) {
        write_file(&sample_text,text,15);
    }
    return 0;*/
    ...
    *((unsigned long int*)file_size) = to_send.file_len;
    ...
    if(to_send.file_len < 1000000) {
        ...
        while(total_bytes_sent < to_send.file_len) {
            ...
            while(bytes_sent < file_block) {
                if(file_block - bytes_sent > 256) {
                    ...
                    send_data(&sock,file_holder+bytes_sent,file_block - bytes_sent,p->ai_addr,p->ai_addrlen,&send_index);
                    bytes_sent = file_block;
                    printf("%ul bytes sent\n",bytes_sent);
                }
            }
            free(file_holder);
    ...
    printf("Expected Bytes Sent: %ul\n",to_send.file_len);
    printf("Expected Bytes Sent 2: %ul\n",ftell(to_send.fileptr));
    return 0;
